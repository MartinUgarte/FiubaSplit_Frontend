...
export default function FilterGroupModal({ open, onClose, selectedFilters, setSelectedFilters, submitFilters }: FilterGroupModalProps) {
            ...
            <Box component="form" display='flex' flex='1' flexDirection='column' justifyContent='center' alignItems='center' sx={style}>
                ...
                <Box display='flex' flexDirection='column' flex='0.8' justifyContent='center' alignItems='center'>
                    ...
                    <TextField
                        ...
                        onChange={(group) => setSelectedFilters(prevState => ({ ...prevState, description: group.target.value as string }))}
                        sx={{ marginTop: 2 }}
                    >
                        Descripción
                    </TextField>
                    <TextField
                        ...
                        select
                        label='Categoria'
                        onChange={(group) => setSelectedFilters(prevState => ({ ...prevState, category: group.target.value as string }))}
                        sx={{ marginTop: 2 }}
                    >
